 ...
 */
 ...
 /* Usage:
    gcc -g audisp-fileWatch.c -o audisp-simple -lauparse -laudit
    cp -pr audisp-simple /usr/local/sbin/
    
    cat /etc/audisp/plugins.d/simple_example.conf 
...
active = yes
direction = out
path = /usr/local/sbin/audisp-simple
type = always 
args = 1
format = string
    ...
    auditctl -w  /tmp/f -p wra
    touch /tmp/f
    echo aaa > /tmp/f
    unlink /tmp/f
    cat /tmp/cgroup_file_miss.log 
...
char *logPath1 = "/dev/null";
char *logPath2 = "/tmp/cgroup_file_miss.log";
FILE * fp1 = NULL;
FILE * fp2 = NULL;
...
int main(int argc, char *argv[])
{
	...
	fp1 = fopen(logPath1, "w+");
	if (fp1 == NULL)  {
		printf("ERROR %d\n", errno);
	}

	fp2 = fopen(logPath2, "w+");
	if (fp2 == NULL)  {
		printf("ERROR %d\n", errno);
	}
...
static void dump_whole_event(auparse_state_t *au, FILE* fp)
{
...
static void dump_fields_of_record(auparse_state_t *au, FILE* fp)
{
	int processed_fileds = 0;
	...
	fprintf(fp, "event time: %u.%u:%lu - %s, host=%s\n", (unsigned)e->sec,
		e->milli, e->serial, strtok(ctime(&(e->sec)), "\n"), e->host ? e->host : "?");
		auparse_first_field(au);

	do {
		fprintf(fp, "field %02d: %s=%s (%s)\n",
		processed_fileds++,
...
{
	const char *ptr1 = audit_msg_type_to_name(auparse_get_type(au));
	const char *ptr2 = auparse_get_record_text(au);
	if (strstr(ptr2, "DELETE") != NULL) {
		fprintf(fp2, "%s: %s\n", ptr1, ptr2);
		fprintf(fp2, "\n");
		dump_fields_of_record(au, fp2);
		dump_whole_event(au, fp2);
		fflush(fp2);
	}
}
		...
		auparse_cb_event_t cb_event_type, void *user_data)
{
	...
	fprintf(fp1, "v--> TOTAL %d EVENTS HANDLED\n\n", num);
}
