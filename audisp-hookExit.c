...
#define PTRACER "/usr/local/bin/ptraceEXIT"
...
static void dump_fields_of_record(auparse_state_t *au, FILE* fp)
{
	...
	do {
		...
		if(AUDIT_PID == auparse_get_field_type(au) && !strncmp(auparse_get_field_name(au), "pid", 3)) {
